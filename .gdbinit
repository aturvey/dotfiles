printf ".gdbinit: type 'helpme' for a list of the user defined functions \n"
...
if $COLOUREDPROMPT == 1
    # set prompt to a green color, be sure to keep a trailing space on the line
    set prompt \033[32m(gdb) \033[0m 
    #set prompt \033[0;32;1m(gdb) \033[0m
end
...
# __________________start user help_________________
...
define helpme
    ...
    printf "    bpm -- Set breakpoint on expression \n"
    printf "    bpt -- Set temporary breakpoint \n"
    printf "    bhb -- Set hardware assisted breakpoint \n"
    printf "    bht -- Set temporary hardware assisted breakpoint \n"
    printf "\n"
    printf "Process Information:\n"
    printf "    argv -- Print program arguments \n"
    printf "    stack -- Print backtrace of the call stack, or innermost COUNT frames \n"
    printf "    frame -- Print stack frame \n"
    printf "    flags -- Print FLAGS register \n"
    printf "    eflags -- Print EFLAGS register \n"
    printf "    reg -- Print CPU registers \n"
    printf "    func -- Print all function names in target, or those matching REGEXP \n"
    printf "    var -- Print all global and static variable names (symbols), or those matching REGEXP \n"
    printf "    lib -- Print shared libraries linked to target \n"
    printf "    sig -- Print what debugger does when program gets various signals\n"
    printf "    threads -- Print threads in target \n"
    printf "    dis -- Disassemble as specified section of memory \n"
    printf "\n"
    printf "Process Control:\n"
    printf "    n -- Step one (or NUM) instruction, but proceed through subroutine calls \n"
    printf "    go -- Step exactly one (or NUM) instruction. Alias for stepi \n"
    printf "    init -- Run program and break on _init() \n"
    printf "    main -- Run program and break on main() \n"
    printf "    pret -- Execute until selected stack frame returns (step out of current call) \n"
    printf "    start -- Run program and break on _start() \n"
    printf "    sstart -- Run program and break on __libc_start_main() \n"
    printf "\n"
    printf "Process Context:\n"
    printf "    context -- Print context window, i.e. regs, stack, ds:esi and disassemble cs:eip \n"
    printf "    context_on -- Enable display of context on every program break \n"
    printf "    context_off -- Disable display of context on every program break  \n"
    printf "\n"
    printf "Edit FLAGS: \n"
    printf "    cfa -- Change Auxiliary Carry flag \n"
    printf "    cfc -- Change Carry flag \n"
    printf "    cfd -- Change Direction flag \n"
    printf "    cfi -- Change Interrupt flag \n"
    printf "    cfn -- Change Negative (Less Than) flag \n"
    printf "    cfo -- Change Overflow flag \n"
    printf "    cfp -- Change Parity flag \n"
    printf "    cfs -- Change Sign flag \n"
    printf "    cft -- Change Trap flag \n"
    printf "    cfv -- Change Overflow flag \n"
    printf "    cfz -- Change Zero flag \n"
    printf "\n"
    printf "Patch: \n"
    printf "    nop -- Patch a single byte at address ADDR1, or a series of bytes between ADDR1 and ADDR2 to a NOP instruction \n"
    printf "    null -- Patch a single byte at address ADDR1, or a series of bytes between ADDR1 and ADDR2 to NULL (0x00) \n"
    printf "    int3 -- Patch byte at ADDR to an INT3 (0xCC) instruction or the equivalent software breakpoint for ARM \n"
    printf "    rint3 -- Restore the original byte previous to int3 patch issued with 'int3' command \n"
    printf "\n"
    printf "Hex/ASCII Dump: \n"
    printf "    ascii_char -- Print ASCII value of byte at address ADDR, or '.' if the value is not printable \n"
    printf "    hex_quad -- Print eight hexadecimal bytes starting at address ADDR \n"
    printf "\n"
    printf "Data Window: \n"
    printf "    dd -- Display 16 lines of a hex dump of address starting at ADDR \n"
    printf "    ddump -- Display NUM lines of hex dump for address in $data_addr global variable \n"
    printf "    datawin -- Display valid address from one register in data window (ESI, EDI, EAX, or ESP) \n"
    printf "\n"
    printf "Miscellaneous: \n"
    printf "    assemble -- Assemble instructions to binary opcodes using NASM \n"
    printf "    asm -- Shortcut to the assemble command \n"
    printf "    assemble_gas -- Assemble instructions to binary opcodes using GNU as and objdump \n"
    printf "    dump_hexfile -- Write a range of memory to a file in Intel ihex (hexdump) format \n"
    printf "    dump_binfile -- Write a range of memory to a binary file \n"
    printf "    cls -- Clear the screen \n"
    printf "    search -- Search for the given parameter between $start and $end address \n"
    if $COLOUREDPROMPT == 1
        printf "[0m\n"
    else
        printf "\n"
...
document helpme
Provide an organized list of user defined functions.
Usage: helpme
end

# __________________end user help___________________
